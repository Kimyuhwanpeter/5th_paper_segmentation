...
def DeepLabV3Plus(img_height, img_width, nclasses=66):
    ...
    y_b = Conv2D(filters=48, kernel_size=1, padding='same',
                 kernel_initializer='he_normal', name='low_level_projection_2', use_bias=False)(y_b)
    y_b = BatchNormalization(name='bn_low_level_projection_2')(y_b)
    y_b = Activation('relu', name='low_level_activation_2')(y_b)

    y = concatenate([x_a, y_b], name='decoder_concat_2')

    y = Conv2D(filters=256, kernel_size=3, padding='same', activation='relu',
               kernel_initializer='he_normal', name='decoder_conv2d_3', use_bias=False)(y)
    y = BatchNormalization(name='bn_decoder_3')(y)
    y = Activation('relu', name='activation_decoder_3')(y)

    y = Conv2D(filters=256, kernel_size=3, padding='same', activation='relu',
               kernel_initializer='he_normal', name='decoder_conv2d_4', use_bias=False)(y)
    y = BatchNormalization(name='bn_decoder_4')(y)
    y = Activation('relu', name='activation_decoder_4')(y)
    y = Upsample(y, [img_height, img_width])

    y = Conv2D(nclasses, (1, 1), name='output_layer2')(y)

    model = Model(inputs=base_model.input, outputs=y, name='DeepLabV3_Plus2')
    model.load_weights("C:/Users/Yuhwan/Downloads/last_epoch.h5")

    model = tf.keras.Model(inputs=base_model.input, outputs=tf.concat([x, y], -1))
